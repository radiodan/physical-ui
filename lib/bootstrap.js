...
function createRGBLEDs(config, opts) {
  ...
  listenForCommands(messagingClient, topicKey, config.id, function (data) {
    ...
    var colour     = data.content.colour,
        transition = data.content.transition;

    logger.info('Command for %s - color %s, transition', config.id, colour, transition);

    if (colour) {
      rgb.colour(colour, transition)
         .then(
