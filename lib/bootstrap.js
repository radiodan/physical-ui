...
function createButtons(config, opts) {
  ...
  button.on('hold', function (evt) {
    ...
    messagingClient.sendToExchange(
      exchange, topicKey + '.hold', { pressed: true, durationMs: evt.durationMs }
    );
  });
...
function createRGBLEDs(config, opts) {
  ...
  listenForCommands(messagingClient, topicKey, config.id, function (data) {
    try {
    ...
    var stateChangePromise = null;

    if (action === 'change') {
      stateChangePromise = data.content.queue.reduce(function (sequence, params) {
        return sequence.then(function () {
          return changeRgbState(rgb, params);
        });
      }, promise.resolve());
    } else {
      stateChangePromise = changeRgbState(rgb, data.content);
    }

    stateChangePromise.then(
      respondToCommand(messagingClient, replyToQueue, correlationId),
      respondToCommand(messagingClient, replyToQueue, correlationId, true)
    );
...
function changeRgbState(rgb, params) {
  ...
  logger.info('Command - color %s, transition', colour, transition, rgb);
...
function respondToCommand(messagingClient, replyToQueue, correlationId, error) {
