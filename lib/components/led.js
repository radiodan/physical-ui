...
function create(pin, opts) {
  ...
  instance.brightness = function (value, localTransition, isTransitioning) {
    var initialValue = values.off * pwmRange,
        dfd = promise.defer(),
        transition = merge(globalTransitions, localTransition);
    ...
    if (transition.duration && !isTransitioning) {
      ...
      activeTween = new tween.Tween({ value: brightness })
                ...
                .repeat(
                  (transition.repeat === true || transition.yoyo === true)
                    ? Infinity
                    : (transition.repeat || transition.yoyo)
                )
                .yoyo(transition.yoyo ? true : false)
                .onUpdate(function () {
